...
only positive effect over the untrained base ($+4.4$\,pp), robust across
...
reproduce \armC{}'s effect on AIME-2024 maj@8 ($\armC{}-\armD{} = +5.58$\,pp,
$95\%$ bootstrap CI $[+1.13, +11.10]$, excluding $0$), refuting a pure
...
$+7.5$\,pp on AMC-23 \texttt{pass@1} over its untrained base. The apparent
$22.5$\,pp gap to Open-RS's reported $80$\,\% headline (\texttt{maj@k})
dissolves under like-for-like \texttt{maj@4} comparison: we obtain $75.0$\,\%
\texttt{maj@4} on Open-RS's own public RS2 checkpoint, within $5$\,pp of
their $80$\,\% (Section~\ref{sec:method}).
...
Hardware: 1$\times$ NVIDIA A100-SXM4-80\,GB on commodity cloud infrastructure.
...
across the three seeds (Tables~\ref{tab:main_v2} and~\ref{tab:delta_v2}).
...
CI $[+3.3, +6.7]$ and \armC{}$-\armD{}$ has CI $[+1.13, +11.10]$, both
...
Table~\ref{tab:main_v2} reports pass@1 on the three benchmarks
...
\begin{figure*}[t]
  ...
  values are listed in Table~\ref{tab:main_v2}; a per-claim verification
...
relative to base (Table~\ref{tab:delta_v2}). This is consistent with
...
\subsection{Ablation \armD{} (constant-bias reward, 3 seeds)}
...
deterministically returns $1.0$ for every input. If the hypothesis is
correct, \armD{} should reproduce \armC{}'s regularization signature on
AIME-2024 maj@8 within seed variance. Three-seed results (\texttt{42, 123, 7}):
\armD{} achieves AMC-23 maj@4 $=70.0\pm 2.5$\,\%, MATH-500 $=61.2\pm 0.9$\,\%,
AIME-2024 pass@1 $=24.4\pm 1.9$\,\%, and AIME-2024 maj@8 $=32.2\pm 5.1$\,\%
(below \armC{}'s $37.8\pm 1.9$\,\%; $\armC{}-\armD{} = +5.58$\,pp,
$95\%$ bootstrap CI $[+1.13, +11.10]$ excluding $0$ over $10{,}000$
subject-bootstrap resamples). \armD{} is not distinguishable from base on
AIME-2024 maj@8 ($\Delta=-1.08\pm 5.07$\,pp; CI bao $0$). The hypothesis is
refuted: $R_5$'s contribution is content-specific, not a pure
reward-magnitude perturbation. The implementation, configuration, and unit
tests for \armD{} are included in the released codebase.
...
($+4.4\pm 1.9$\,pp over base; $95\%$ bootstrap CI $[+3.3, +6.7]$) and the
\armC{}-vs-\armD{} contrast ($+5.58$\,pp, $95\%$ bootstrap CI
$[+1.13, +11.10]$) are both statistically distinguishable from zero under
...
The original $12.9$\,pp gap was a metric-mismatch artifact, not a pipeline bug.
...
answers to fail string-matching. Both fixes are in the v2 commit; the
post-fix \texttt{maj@4} numbers are reported in Table~\ref{tab:main_v2}.
